...
if (isset($_POST['deleteAlbumBtn'])) {
    ...
    $getPhotosStmt = $pdo->prepare("SELECT photo_name FROM photos WHERE album_id = :album_id");
    $getPhotosStmt->bindParam(':album_id', $album_id, PDO::PARAM_INT);
    $getPhotosStmt->execute();
    $photos = $getPhotosStmt->fetchAll();

    foreach ($photos as $photo) {
        $filePath = "../images/" . $photo['photo_name'];
        if (file_exists($filePath)) {
            unlink($filePath); 
        }
    }

    $deletePhotosStmt = $pdo->prepare("DELETE FROM photos WHERE album_id = :album_id");
    $deletePhotosStmt->bindParam(':album_id', $album_id, PDO::PARAM_INT);
